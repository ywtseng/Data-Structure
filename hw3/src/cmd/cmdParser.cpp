...
{
   // TODO...
   map<const string, CmdExec*>:: const_iterator at;
   for(at=_cmdMap.begin();at!=_cmdMap.end();at++){
      (at->second)->help();
   }
   cout<<endl;
}
...
{
   // TODO...
   vector<string>                               _commandName;
   vector<string>             :: const_iterator bt;
   vector<string>                               _match_string;
   map<const string, CmdExec*>:: const_iterator at;
   int counter;
   bool _temp_after_match=0;
   string cmd;
   //
   myStrGetTok(str,cmd);
   for(at=_cmdMap.begin();at!=_cmdMap.end();at++){
       string temp=at->first+at->second->getOptCmd();
      _commandName.push_back(temp);
   }
   //Condition 1
   if(str.empty()){
      for(bt=_commandName.begin(),counter=0;bt!=_commandName.end();bt++,counter++){
         if(counter%5==0)
            cout<<endl;
         cout<<setw(12)<<left<<*bt;
      }
      //print next command
      reprintCmd();
   }
   //---------------------
      ...
      _match_string.clear();
      for(bt=_commandName.begin();bt!=_commandName.end();bt++){
         string temp_b=*bt;
         string temp_c=temp_b.substr(0,str_size);
         if(myStrNCmp(str,temp_c, temp_c.size())==0){
            if(str_size>temp_b.size())
               _temp_after_match=1;
            else
               _match_string.push_back(temp_b);
         }
      }
      if(_temp_after_match==1){
      ...
      //Condition 4
      else if(_match_string.size()==0&&_temp_after_match==0){
         mybeep(); 
         return;
      }
      //Condition 3
      else if(_match_string.size()==1&&_temp_after_match==0){
         string temp_d=_match_string[0];
         string temp_e=temp_d.substr(str_size,temp_d.size());
      ...
      //Condition 2
         ...
         for(bt=_match_string.begin(),counter=0;bt!=_match_string.end();bt++,counter++){
            if(counter%5==0)
               cout<<endl;
            cout<<setw(12)<<left<<*bt;
         }
...
CmdParser::getCmd(string cmd)
{
   ...
   for(at=_cmdMap.begin();at!=_cmdMap.end();at++){
      string temp=at->first+at->second->getOptCmd();
      int num_compare=at->first.size();
      if(myStrNCmp(temp,cmd,num_compare)==0)
         e=at->second;
   }
   //------------------
   return e;
