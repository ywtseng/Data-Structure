...
#define toSizeT(t)      (t%SIZE_T)?((t/SIZE_T)+1)*SIZE_T:((t/SIZE_T)*SIZE_T) // TODO
//
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(_stop==0){
         if(ll->getArrSize()==n)
            return ll;
         if(ll->getNextList()!=NULL)
            ll=ll->getNextList();
         else
            _stop=1;
      }
      MemRecycleList<T>* _temp=new MemRecycleList<T>(n);
      ll->setNextList(_temp);
      ll=ll->_nextList;
      return ll;
   }
   ...
   T* getMem(size_t t) {
      ...
      size_t _array_size=getArraySize(t);
      MemRecycleList<T> *ll=getMemRecycleList(_array_size);
      ret=ll->popFront();
      if(ret==NULL){
         bool sufficient=_activeBlock->getMem(t,ret);
         if(sufficient==false){
            size_t rm=_activeBlock->getRemainSize();
            if(rm>=S){
               size_t rn=getArraySize(rm);
               _activeBlock->getMem(rm,ret);
               #ifdef MEM_DEBUG
               cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
               #endif // MEM_DEBUG
   ...
   size_t getNumBlocks() const {
      ...
      bool _stop=0;
      while(_stop==0){
         if((_temp=_temp->_nextBlock)==NULL)
            _stop=1;
         count++;
      }
      return count;
